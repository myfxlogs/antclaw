...
SELECT create_hypertable('vix_term_structure', 'time', chunk_time_interval => INTERVAL '90 days', if_not_exists => TRUE);
...
SELECT create_hypertable('dvol_snapshots', 'time', chunk_time_interval => INTERVAL '30 days', if_not_exists => TRUE);
...
SELECT create_hypertable('gex_snapshots', 'time', chunk_time_interval => INTERVAL '30 days', if_not_exists => TRUE);
...
SELECT create_hypertable('iv_skew_history', 'time', chunk_time_interval => INTERVAL '90 days', if_not_exists => TRUE);
