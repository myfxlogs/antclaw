...
CREATE INDEX IF NOT EXISTS idx_treasury_auctions_date ON treasury_auctions (auction_date DESC);

-- 世界银行宏观数据
CREATE TABLE IF NOT EXISTS worldbank_macro (
    country VARCHAR(8),
    currency VARCHAR(8),
    year INT,
...
-- IMF WEO 数据
CREATE TABLE IF NOT EXISTS imf_weo (
    country VARCHAR(8),
...
SELECT create_hypertable('fetch_jobs', 'started_at', chunk_time_interval => INTERVAL '7 days', if_not_exists => TRUE);

CREATE INDEX IF NOT EXISTS idx_fetch_jobs_source ON fetch_jobs (source, started_at DESC);
CREATE INDEX IF NOT EXISTS idx_fetch_jobs_status ON fetch_jobs (status, started_at DESC);
