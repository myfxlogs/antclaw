...
CREATE INDEX idx_price_daily_symbol ON price_daily (symbol, time DESC);
CREATE INDEX idx_price_daily_source ON price_daily (source, time DESC);
...
SELECT add_compression_policy('price_intraday', INTERVAL '30 days');

CREATE INDEX idx_price_intraday_symbol ON price_intraday (symbol, interval, time DESC);
