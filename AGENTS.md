...
- **写代码前，先扫重复模式**：
  1. **骨架铺开**：所有同类文件的函数签名（只签名，不实现）同时写出来；
  2. **扫重复信号**：同一模式出现 ≥ 2 次且逻辑 > 3 行 → **立即提取公共函数/文件**，不等第三次；
  3. **按职责分文件**：如果某段逻辑不属于当前文件名的职责（例如推送检测器不应该包含"如何分页扫描用户"），移到基础设施层或 `*_util` 文件；
  4. **再写实现**：每个文件的业务逻辑直接依赖已提取的公共层，避免写完再重构。
  - 常见重复信号：分页扫描循环、去重→发送→记录三步模板、时间范围计算、货币/影响级别过滤。
